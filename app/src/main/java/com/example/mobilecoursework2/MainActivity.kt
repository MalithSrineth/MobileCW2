...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val btnSearchMeals: Button = findViewById(R.id.btnSearchMeals)
        val btnAddMeals: Button = findViewById(R.id.btnAddMeals)
        val btnSearchByIngredient: Button = findViewById(R.id.btnSearchByIngredient)

        val db2 = Room.databaseBuilder(applicationContext, MealDatabase::class.java, "meal-db").build()
        ...
        btnAddMeals.setOnClickListener {
            //print("Hello")
            val meal1 = Meal(
                meal = "Sweet and",
                drinkAlternate = null,
            ...
            CoroutineScope(Dispatchers.IO).launch {
                mealDao.clear()
                mealDao.resetPrimaryKey()
                mealDao.insert(meal1)
                mealDao.insert(meal2)
                mealDao.insert(meal3)
                mealDao.insert(meal4)
            }

            CoroutineScope(Dispatchers.IO).launch {
                val meals = mealDao.getAllMealNames()
                withContext(Dispatchers.Main) {
                    // Update UI on the main thread
                    testing.text = meals.joinToString("\n")
                }
            }
        ...
        btnSearchMeals.setOnClickListener {
            val searchForMeals = Intent(this, SearchForMeals::class.java)
            startActivity(searchForMeals)
        }
    }
}